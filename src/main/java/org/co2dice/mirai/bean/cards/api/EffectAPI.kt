...
interface EffectAPI<S:Scene,C:Cards,R:CharacterCard> {
    var function: (S,C,R)->Boolean
    fun cost(scene:S,cards: C,character:R):List<Token>
    //三个传参:场景，调用的卡牌，效果调用者
    //技能入场时的token花费，宣言使用技能后无论怎么样都会消耗。返回一个消耗token的数组。
    fun trigger(scene:S,cards: C,character:R):Boolean
