...
abstract class SkillActive(holder: Cards) : Skill(holder) {
    ...
    abstract var cost:Function4<MutableList<String>,Scene,SkillActive, Cards,List<Token>>
    //使用技能的token花费，宣言使用技能后无论怎么样都会消耗.cards应该为holder，因为通过这张卡可以获得持有者的信息。返回一个消耗token的数组。
    abstract var check:Function3<MutableList<String>,SkillActive,Cards,Boolean>
    //检定值,宣言后会检定是否可以使用技能。cards应该为holder，因为通过这张卡可以获得持有者的信息。
    abstract var react:Function3<MutableList<String>,SkillActive,Cards,Int>
    //反抗值,敌人受到技能影响后会检定是否可以反抗，若成功则豁免.cards应该为技能目标。
    abstract var effect:Function5<MutableList<String>,Scene,SkillActive, Cards, Cards, CAOPot>
    //造成的特效，返回true则技能生效，返回false则技能失效。第一个cards应该为holder，第二个应为target。输出为本技能提供的混乱/秩序值。
    abstract var reactEffect:Function5<MutableList<String>,Scene,SkillActive, Cards, Cards, CAOPot>
    //反抗成功后的特效,第一个应该为holder,第二个应为target.
