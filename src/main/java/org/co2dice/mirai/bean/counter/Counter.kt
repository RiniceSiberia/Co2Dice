...
open class Counter(
    ...
    //token的名称
    open val type: TokenDepend,
    //可以放在什么载体上
    var isTempToken: Boolean = false,
    //是否是临时token
    var lifetime:Int = -1
    ...
    var checkPoint:Function2<Battle, ChessmanInstance,Boolean> = checkPoint@{ _, _ ->
        return@checkPoint isTempToken
    }

    var timeFlow:Function2<Battle, ChessmanInstance,Boolean> = timeFlow@{ _, _ ->
    ...
    fun addEvent(fuller: CounterFuller):Boolean{
        //添加事件
        return true
    }
    fun removedEvent(fuller: CounterFuller):Boolean{
        //删除事件
        return true
    }
    fun canReplace(counter: Counter):Boolean{
        //检查是否可以替代传参中的token
        return this == counter
    }
}