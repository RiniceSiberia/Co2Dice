...
class CounterFuller (val counterType: Counter,
    ...
    //重复添加n个point进points中
    val counters : MutableList<Counter> = mutableListOf<Counter>().apply{
    ...
    fun timePointCheck(battle : Battle, chessman: ChessmanInstance):Boolean{
        //对每个点都执行一次检查
        counters.forEach{
    ...
    fun addToken(counter: Counter):Boolean{
        if (!counter.isTempToken){
            return if (this.getPoints() + 1 < this.limit && counter.canReplace(this.counterType)){
