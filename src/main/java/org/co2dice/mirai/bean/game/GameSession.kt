...
class GameSession (
    ...
    //	创建会话时 MPSD 分配的会话 ID。 当访问存储在会话中的信息时，此值通常包含在 URI 中。
...
//    //谁可以踢掉其他玩家
    ...
    var visibility: VisibilityLevelEnum = VisibilityLevelEnum.PUBLIC,
    //能如何访问这个会话
    var password:String? = null,
    //密码，如果有密码非空则必须要输入密码才能加入游戏，和会话无关
    val dependId:String?
    //决定这个对话是否有一个公屏来显示公开信息展示公开信息，为空则是向所有参与者发私信
    ){

    var seatsAvailable:Int = maxPlayer - roster.size

    enum class VisibilityLevelEnum{
        //本游戏的会话有:
        // 公开(可以在群组或私聊中进行游戏，可以通过指令获取会话列表)，
        // 私有(可以在群组或私聊中进行游戏，只有输入会话的id才能加入)，
        // 随机(匹配模式，可以随机匹配的加入其他人的游戏，只能私聊中进行)
        PUBLIC, PRIVATE, RANDOM
    }
