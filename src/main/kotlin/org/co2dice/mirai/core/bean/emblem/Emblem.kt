...
import org.co2dice.mirai.core.bean.ability.instance.FieldAbilityInstance
import org.co2dice.mirai.core.bean.api.agent.ActivatedAgent
import org.co2dice.mirai.core.bean.api.PermanentInstance
import org.co2dice.mirai.core.bean.activated.instance.FieldEffectInstance
 ...
 * @Message: Emblem(徽记)来自万智牌的徽记，他可以放在人物，物品，场地上。
 * 徽记是永存的，除非依附的卡离场或者依附的人物无法继续行动，它不会被清除。
 * 徽记可以释放效果。徽记释放的效果必须通过被依附的物体实现。
 * 徽记本身就是instance,不存在原型
 **/
class Emblem<T : PermanentInstance>(
    var activatedAbilities: List<FieldEffectInstance>,
    override var abilities: List<FieldAbilityInstance>,
    registry : UniqueIdRegistry,
) : ActivatedAgent<FieldEffectInstance, FieldAbilityInstance>,PermanentInstance{
