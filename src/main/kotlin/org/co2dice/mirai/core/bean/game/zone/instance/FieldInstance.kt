...
class FieldInstance(private val size : @IntRange(from = 0, to = VENUE_SIZE_MAX.toLong())Int = VENUE_SIZE_MAX,
    ...
    val chessmen : MutableMap<FieldChessmanInstance, ChessmanValue> = mutableMapOf()
    //一个玩家可以拥有不止一个棋子，当失去所有棋子或所有棋子都无法行动时,游戏失败,除非是系统棋子
    //Equipments(装备）是和棋子绑定的

    fun randomChunks(venues : List<VenueCardInstance>) : FieldInstance {
    ...
    private fun chessMove(coordinate: Coordinate2D, chessman: ChessmanInstance) : Boolean{
        ...
        return venueCanStand(coordinate)
            //目标可去
            && chessmen[chessman]?.let { it.coordinate != coordinate } ?: false
            //为空时返回false,不为空时坐标不相同返回true,相同返回false
            && chessmen[chessman].also { it!!.coordinate = coordinate } != null
        //移动棋子
    }
    ...
    private fun getVenueChessNum(coordinate: Coordinate2D) : Int{
        //获取指定位置的棋子数量
        return chessmen.values.stream().filter { it.coordinate == coordinate }.count().toInt()
    }
